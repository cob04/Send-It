...
from app.api.version2.models import CANCELLED, NOT_DELIVERED, IN_TRANSIT
from app.api.version2.models import parcel_orders
...
class ParcelOrderEndpointsTests(unittest.TestCase):
    ...
    def test_get_by_id_when_order_exists(self):
        self.app.post('/api/v2/parcels',
                      data=json.dumps(self.data),
                      content_type="application/json")
        response = self.app.get('/api/v2/parcels/1')
        self.assertEqual(response.status_code, 200)
        expected_json = {
            "message": "Success",
            "parcel_order": {
    ...
    def test_get_by_id_when_order_does_not_exist(self):
        response = self.app.get('/api/v2/parcels/2')
    ...
    def test_cancelling_an_order(self):
        post_response = self.app.post('/api/v2/parcels',
                                      data=json.dumps(self.data),
                                      content_type="application/json")
        ...
        put_response = self.app.put('/api/v2/parcels/1/cancel',
    ...
    def test_cancelling_an_order_with_wrong_order_id(self):
        ...
        put_response = self.app.put('/api/v2/parcels/3/cancel',
                                    data=json.dumps(data),
                                    content_type="application/json")
        data["id"] = 1
        expected_json = {
